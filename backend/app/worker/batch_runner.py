...
async def run_batch_async(batch_id: int, *, stop_on_error: bool = True) -> dict[str, Any]:
    async with async_session_maker() as session:
        ...
        for task in tasks:
            ...
            try:
                await set_task_status(session, task, BatchTaskStatus.running, error=None)
                ...
                await set_task_status(session, task, BatchTaskStatus.done, error=None)
            except Exception as error:  # noqa: BLE001
                failed_count += 1
                logger.exception("Ошибка выполнения task_id=%s batch_id=%s", task.id, batch_id)
                await set_task_status(session, task, BatchTaskStatus.failed, error=str(error))
                if stop_on_error:
